      ...
      switch (change.operation) {
        case 'add':
          updateApprovedForProvisioning(self, entry.object.targetdn,
                                        change.modification.vals, function() {});
          break;
        default:
          throw new Error('default case invoked');
      }
...
function add(self, changes, entry, cb) {
  ...
  function addUser(self, changes, entry, cb) {
    ...
    var payload = {
      uuid: changes.uuid[0],
      // MANTA-1289
      approved_for_provisioning: changes.approved_for_provisioning ? changes.approved_for_provisioning[0] : null
    };
  ...
  function addGroup(self, changes, entry, cb) {
    ...
    /*
     * MANTA-1194: add group entries may not contain members, in this case we
     * just skip it -- since there will be entries whenever a user is added to
     * a group, and that is the changelog mahi will parse and use.
     */
