...
util.inherits(Mahi, util.EventEmitter);
...
Mahi.prototype.poll = function () {
    ...
    function transform(err, entry) {
        ...
        vasync.pipeline({
            funcs: [
                function toRedis(cb) {
                    self.log(entry, 'sending entry to redis');
                ...
                function updateChangenumber(cb) {
                    self.updateChangenumber(entry.object.changenumber,
...
Mahi.prototype.updateChangenumber = function (changenumber, cb) {
    ...
    self.log.info({
        ...
        currentChangenumber: self.changenumber
