...
function add(opts, cb) {
    ...
    /*
     * Check for a revocation tombstone before writing.
     * If an operator called POST /scope-revoke for this key,
     * a tombstone exists in Redis with a TTL.  While the
     * tombstone is present, skip writing the key to Redis
     * so the revocation is durable across replication cycles.
     *
     * DECISION: fail-closed on tombstone check errors.
     * If the Redis GET for the tombstone fails, we abort
     * the replication for this entry rather than proceeding.
     * Rationale: a revoked key re-appearing in Redis is a
     * security violation; a delayed replication retry is
     * not.  The replicator's backoff/retry handles transient
     * Redis errors.  If this proves too aggressive under
     * sustained Redis pressure, switch to fail-open (log
     * warn, proceed with write).
     */
    var revokedKey = akFormat.revokedKeyPath(accesskeyid);
    redis.get(revokedKey, function (revErr, revoked) {
        if (revErr) {
            cb(revErr);
            return;
        }
        if (revoked) {
            log.info({ accesskeyid: accesskeyid },
                'accesskey.add: skipping revoked key' +
                ' (tombstone present)');
            cb(null, batch);
            return;
        }
    ...
    redis.get(key, function (err, res) {
        ...
        /*
         * Version check: skip the write if the existing entry
         * has a higher version (e.g. written by cachePush).
         * Absent version = 0.
         *
         * INVARIANT: version values are comparable by > only
         * because cachePush uses Date.now() (~10^12) and the
         * replicator uses changenumber (~10^3–10^6).  cachePush
         * always wins, which is correct since it carries the
         * freshest data.  If either domain changes, this
         * comparison breaks.
         */
        var changenumber = parseInt(entry.changenumber, 10);
        // if the changenumber invariant is broken we need
        // to know it.
        assert.ok(Number.isFinite(changenumber) &&
            changenumber > 0,
          'changenumber must be > 0, received: ' +
            entry.changenumber);

        /*
         * Sanity check: changenumber should always be
         * well below Date.now() (~10^12).  If it ever
         * exceeds 10^9 the version comparison with
         * cachePush timestamps breaks silently.
         */
        if (changenumber > 1e9) {
            log.error({
                changenumber: changenumber,
                accesskeyid: accesskeyid
            }, 'accesskey.add: changenumber exceeds' +
                ' safety ceiling (version comparison' +
                ' with cachePush may be broken)');
        }
        ...
        if (credentialType === 'permanent') {
            ...
            if (existingKeyData &&
                typeof (existingKeyData) === 'object') {
                existingVersion =
                    existingKeyData.version || 0;
            }
            if (existingVersion > changenumber) {
                log.info({
                    accesskeyid: accesskeyid,
                    existingVersion: existingVersion,
                    changenumber: changenumber
                }, 'accesskey.add: skipping stale' +
                    ' write (existing version newer)');
                cb(null, batch);
                return;
            }
        }
        ...
        if (credentialType === 'temporary') {
             ...
             * with secret, scope, and version.  Unscoped keys
             * use scope: null.  This unified format eliminates
...
function modify(opts, cb) {
    ...
    redis.get(revokedKey, function (revErr, revoked) {
        ...
        if (revoked) {
            log.info({ accesskeyid: accesskeyid },
                'accesskey.modify: skipping revoked key' +
                ' (tombstone present)');
            cb(null, batch);
            return;
        }
    ...
    redis.get(key, function _redisGet(err, res) {
        ...
        /*
         * Version check: skip the write if the existing entry
         * has a higher version (e.g. written by cachePush with
         * a newer changenumber).
         */
        var changenumber = parseInt(entry.changenumber, 10);
        assert.ok(Number.isFinite(changenumber) &&
            changenumber > 0,
            'changenumber must be a positive integer, got: ' +
            entry.changenumber);
        if (changenumber > 1e9) {
            log.error({
                changenumber: changenumber,
                accesskeyid: accesskeyid
            }, 'accesskey.modify: changenumber exceeds' +
                ' safety ceiling (version comparison' +
                ' with cachePush may be broken)');
        }
        var existingKeyData = payload.accesskeys ?
            payload.accesskeys[accesskeyid] : null;
        var existingVersion = 0;
        if (existingKeyData &&
            typeof (existingKeyData) === 'object') {
            existingVersion =
        ...
        if (existingVersion > changenumber) {
            log.info({
                ...
                existingVersion: existingVersion,
                changenumber: changenumber
            }, 'accesskey.modify: skipping stale' +
                ' write (existing version newer)');
            cb(null, batch);
            return;
        }
