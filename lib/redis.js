...
function connect(opts, cb) {
        ...
        dns.resolve4(opts.host, function (dns_err, addrs) {
                if (dns_err) {
                        log.debug(dns_err, 'redis: DNS call failed');
                        cb(dns_err);
                        return;
                } else if (!addrs || addrs.length === 0) {
                        log.debug('redis: no entries in DNS');
                        cb(new Error('redis not in DNS'));
                        return;
                }
                ...
                log.debug({
                        host: opts.host,
                        addresses: addrs
                }, 'resolved redis in DNS');
                var client = redis.createClient(opts.port,
                                                shuffle(addrs).pop(),
                                                opts.options);
                var t;
