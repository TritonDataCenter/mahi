...
function verifySigV4(opts, cb) {
        ...
        log.debug({
                ...
                accessKeyIdLength: authInfo.accessKeyId ? authInfo.accessKeyId.length : 0,
                accessKeyIdHex: authInfo.accessKeyId ? new Buffer(authInfo.accessKeyId).toString('hex') : null,
        ...
        redis.get(accessKeyLookupKey, function (err, userUuid) {
                ...
                redis.get(userKey, function (err, userRes) {
                        ...
                        // URL decode the originalUrl if it comes from query params
                        // (fixes Cyberduck compatibility without affecting AWS CLI)
                        if (req.query.url) {
