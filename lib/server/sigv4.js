 ...
 * it is absent from Redis and no UFDS client is available.
 * If the key subsequently arrives in Redis (via replication
 * or cache-push), the cache entry will still cause 401s for
 * up to NEGATIVE_CACHE_TTL_MS (30 seconds).
 *
 ...
 *
 * Clears the entire cache if it exceeds
 * NEGATIVE_CACHE_MAX_SIZE to bound memory under
 * sustained attack with unique key IDs.
 *
 * @param {string} keyId
 */
    ...
    var isTemporaryCredential = sessionToken &&
        ...
        redis.get(accessKeyLookupKey, function (err, lookupVal) {
                ...
                redis.get(userKey, function (userErr, userRes) {
                         ...
                         *
                         * Before commit 1774f55, the replicator
                         * stored unscoped permanent keys as
                         * bare secret strings and scoped keys
                         * as { secret, scope } objects.  Having
                         * two formats forced every consumer to
                         * branch on typeof.
                         *
                         * Commit 1774f55 unified the format so
                         * the replicator writes ALL permanent
                         * keys as objects:
                         *   { secret: "...", scope: <json|null> }
                         * Unscoped keys use scope: null.
                         *
                         * The string fallback below exists
                         * because the replicator only re-writes
                         * a key when its UFDS entry changes.
                         * Dormant keys written before 1774f55
                         * remain as bare strings in Redis until
                         * the key is modified (status toggle,
                         * scope change, rotation) or Redis is
                         * rebuilt.
                         *
                         * Remove this branch once all pre-1774f55
                         * keys have been re-replicated.
                         */
                        var keyData =
                            user.accesskeys[authInfo.accessKeyId];
                        var secretKey;
                        ...
                        // Y2038 detection: Log warning for post-Y2038
                        // timestamps. JavaScript Date arithmetic works
                        // correctly on all platforms, so freshness check
                        // proceeds normally.
                        // TODO: Remove this check after migrating to 64-bit
                        // Node.js (Y2038 only affects 32-bit systems).
                        if (requestTime > Y2038_THRESHOLD_MS) {
                                log.warn({
                                        requestTimestamp: timestamp,
                                        requestTimeMs: requestTime,
                                   ...
                                   'threshold. Timestamp validation proceeds ' +
                                   'normally.');
                        }
