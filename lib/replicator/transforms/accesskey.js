...
function add(opts, cb) {
    ...
    redis.get(key, function (err, res) {
        ...
        if (credentialType === 'temporary') {
        ...
        } else if (accessKeyScope) {
            // Scoped permanent credential: store object with
            // secret and scope
            payload.accesskeys[accesskeyid] = {
                secret: accesskeysecret,
            ...
            // Unscoped permanent credential: keep legacy
            // string format
        ...
        if (credentialType === 'temporary') {
            ...
            // For scoped permanent credentials, store
            // credential data with scope
            var scopedData = {
                type: 'accesskey',
                accessKeyId: accesskeyid,
                userUuid: uuid,
            ...
            log.debug({
                accesskeyid: accesskeyid,
                hasScope: true
            }, 'Storing scoped permanent credential');
        } else {
            // Unscoped permanent: store just UUID
            // (legacy behavior)
