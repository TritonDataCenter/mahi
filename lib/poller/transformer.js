...
function jsonComparator(a, b) {
    // TODO talk to pedro about API for policies
    return (stringComparator(JSON.stringify(a), JSON.stringify(b)));
}
...
function setUnion(opts, cb) {
    ...
    assert.string(opts.key, 'key');
    assert.string(opts.set, 'set');
    assert.array(opts.elements, 'elements');
    assert.object(opts.batch, 'batch');
    assert.object(opts.log, 'log');
    assert.object(opts.redis, 'redis');
    assert.func(cb, 'callback');
    ...
    redis.get(key, function (err, res) {
        ...
        elements.forEach(function (e) {
            var index = payload[set].indexOf(e);
            if (index === -1) {
                payload[set].push(e);
            }
        });
...
function setDifference(opts, cb) {
    ...
    assert.string(opts.key, 'key');
    assert.string(opts.set, 'set');
    assert.arrayOfString(opts.elements, 'elements');
    assert.object(opts.batch, 'batch');
    assert.object(opts.log, 'log');
    assert.object(opts.redis, 'redis');
    assert.func(cb, 'callback');
...
Transformer.prototype.putRole = function (opts, cb) {
    ...
    barrier.on('drain', function barrierDrain() {
        self.log.info({batch: batch.queue}, 'putRole: done');
        cb(error, batch);
    });
...
Transformer.prototype.delRole = function (opts, cb) {
    ...
    barrier.once('drain', function barrierDrain() {
        self.log.info({batch: batch.queue}, 'delRole: done');
        cb(error, batch);
    });
