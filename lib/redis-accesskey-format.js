 ...
 *
 * The replicator transform (UFDS → Redis sync) is the sole
 * in-workspace writer of permanent key data to Redis as of
 * CHG-138; an earlier cachePush endpoint that wrote the same
 * format was removed there. This module remains the single
 * source of truth for the Redis format so the structure stays
 * stable for the sigv4 reader and any out-of-workspace
 * operator tooling that needs to inspect or write entries.
 *
 ...
 *   0 for unversioned).  Used by the replicator to prevent
 *   stale writes from overwriting newer data.
