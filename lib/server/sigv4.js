...
function createCanonicalRequest(method, uri, queryString, headers,
        ...
        if (queryString) {
                var params = queryString.split('&').map(function (param) {
                        ...
                        // Handle empty values correctly - AWS SigV4 spec requires
                        // empty values to be encoded as empty string, not 'undefined'
...
function verifySigV4(opts, cb) {
        ...
        log.debug({
                ...
                accessKeyIdHex: (authInfo.accessKeyId && 
                        ...
                        new Buffer(authInfo.accessKeyId).toString('hex') : null,
