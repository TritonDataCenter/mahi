...
function verifySigV4(opts, cb) {
    ...
    var isTemporaryCredential = sessionToken &&
        ...
        redis.get(accessKeyLookupKey, function (err, lookupVal) {
                ...
                /*
                 * Reverse-lookup values are either a plain UUID
                 * string (legacy unscoped permanent keys) or a
                 * JSON object with a userUuid field (scoped
                 * permanent keys).  Extract the UUID in both cases.
                 */
                var userUuid;
                ...
                redis.get(userKey, function (userErr, userRes) {
                         ...
                         *
                         * Permanent credentials may be stored
                         * as a plain string (legacy/unscoped)
                         * or as an object with secret and scope
                         * fields (scoped credentials).
                         */
                        var keyData =
                            user.accesskeys[authInfo.accessKeyId];
                        var secretKey;
