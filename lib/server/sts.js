...
/*
 * Reduce the caller-supplied bucket scope to the form suitable
 * for the LDAP `accesskeyscope` attribute. Returns undefined
 * when the attribute should be omitted from the LDAP entry:
 *   - the sentinel 'none' is meaningful only to
 *     manta-buckets-api's enforceBucketScope (see
 *     bucket-scope.js:269) and would fail JSON.parse in the
 *     UFDS schema validator (sdc-ufds/schema/accesskey.js:368),
 *   - empty / falsy values are not meaningful in LDAP.
 *
 * The caller (manta-buckets-api) is the only client of this
 * endpoint and is expected to send a JSON scope string or
 * null/undefined; the type is asserted at the entry points.
 */
function ldapScopeAttr(callerBucketScope) {
    if (!callerBucketScope || callerBucketScope === 'none') {
        return undefined;
    }
    return callerBucketScope;
}
...
/**
 * Parses a role ARN to extract account ID and role name.
 *
 * @param {string} roleArn - Role ARN in format arn:aws:iam::account:role/name
 * @returns {Object} {accountId: string, roleName: string} or null if invalid
 */
function parseRoleArnForTrustPolicy(roleArn) {
    ...
    var roleName = resourcePart.substring(5);
    return ({accountId: accountId, roleName: roleName});
}

/**
 * Finds AWS trust policy in UFDS memberpolicy array.
 * Trust policy must have Statement array with sts:AssumeRole action.
 *
 * @param {Array} memberpolicy - Array of policy strings from UFDS
 * @returns {string|null} Trust policy JSON string or null if not found
 */
function findTrustPolicyInMemberpolicy(memberpolicy) {
    if (!memberpolicy || memberpolicy.length === 0) {
        return (null);
    }

    for (var i = 0; i < memberpolicy.length; i++) {
        var policy = memberpolicy[i];
        try {
...
function fetchRoleTrustPolicy(roleArn, ufds, log, callback) {
    ...
    var arnParsed = parseRoleArnForTrustPolicy(roleArn);
    if (!arnParsed) {
        callback(new errors.InvalidParameterError(
            'Invalid role ARN format or ARN must specify a role'));
        return;
    }

    var accountId = arnParsed.accountId;
    var roleName = arnParsed.roleName;
    ...
    ufds.search(searchBase, {
        ...
        result.on('end', function () {
            ...
            if (roles.length === 0) {
                ...
                callback(new errors.NoSuchEntityError(
                    'Role not found'));
                return;
            }
            ...
            var trustPolicy = findTrustPolicyInMemberpolicy(role.memberpolicy);
...
function assumeRole(req, res, next) {
    ...
    /**
     * @type {string} callerBucketScope - Bucket scope
     * inherited from the parent access key.  When the parent
     * key carries a scope JSON string, the temp credential
     * inherits it.  When the parent key is unscoped, the
     * sentinel "none" is stored so that enforceBucketScope
     * can distinguish "parent was unscoped" from "scope was
     * lost in transit."
     *
     * Contract: manta-buckets-api is the only caller of this
     * endpoint and sends either a JSON scope string or
     * null/undefined. Fail fast if that contract is ever
     * violated, which surfaces the buckets-api bug rather
     * than silently corrupting stored credentials. The
     * shipped assert-plus (0.1.5) optionalString only accepts
     * undefined, so we hand-roll the null-permitting check.
     */
    if (caller.bucketScope !== undefined && caller.bucketScope !== null) {
        assert.string(caller.bucketScope, 'caller.bucketScope');
    }
    var callerBucketScope = caller.bucketScope || 'none';
    ...
    log.debug({
        ...
        callerRoleArn: caller.roleArn,
        callerLogin: callerLogin,
        callerBucketScope: callerBucketScope,
        callerKeys: Object.keys(caller)
    ...
    function generateTemporaryCredentials(roleData) {
        ...
        accesskey.generate(accesskey.DEFAULT_PREFIX,
            ...
            try {
                ...
                // Lazy require to avoid circular dependency issue
            ...
            var accessKeyData = {
                ...
                /* bucket scope from parent key (string|null) */
                bucketScope: callerBucketScope,
                created: now
            };
            ...
            /*
             * Only persist the bucket scope LDAP attribute when the
             * parent key actually carried a scope.  ldapScopeAttr
             * drops the 'none' sentinel (which would fail the UFDS
             * schema validator's JSON.parse) and returns undefined
             * for empty values.
             */
            var assumeRoleScopeAttr = ldapScopeAttr(callerBucketScope);
            if (assumeRoleScopeAttr) {
                ldapObject.accesskeyscope = assumeRoleScopeAttr;
            }
            ...
            log.info({
                ...
                ldapObjectAccesskeyscope:
                    ldapObject.accesskeyscope || null,
                callerBucketScope: callerBucketScope,
...
/**
 * Extracts caller identity (UUID and login) from caller object.
 *
 * @param {Object} caller - Caller object with user/account info
 * @returns {Object} {uuid: string, login: string}
 */
function extractCallerIdentity(caller) {
    var callerUuid = caller.user && caller.user.uuid ? caller.user.uuid :
        caller.account.uuid;
    var login = caller.user && caller.user.login ? caller.user.login :
        caller.account.login;
    return ({uuid: callerUuid, login: login});
}

/**
 * Validates and parses DurationSeconds parameter for GetSessionToken.
 *
 * @param {Object} params - Request parameters
 * @param {Object} body - Request body
 * @returns {Object} {valid: boolean, value: number, error: Error|null}
 */
function validateDurationSeconds(params, body) {
    var durationSeconds = parseInt(params && params.DurationSeconds ||
                                   body.DurationSeconds || 3600, 10);
    ...
    return ({valid: true, value: durationSeconds, error: null});
}

/**
 * Creates session token data object for JWT generation.
 *
 * @param {string} callerUuid - Caller UUID
 * @param {Date} expiration - Expiration date
 * @returns {Object} Session token data for JWT
 */
function createSessionTokenData(callerUuid, expiration) {
    return {
        uuid: callerUuid,
        expires: Math.floor(expiration.getTime() / 1000),
...
/**
 * Builds LDAP object for storing session token in UFDS.
 *
 * @param {Object} params - Parameters for LDAP object
 * @param {string} params.accessKeyId - Temporary access key ID
 * @param {string} params.secretKey - Secret access key
 * @param {string} params.sessionToken - JWT session token
 * @param {Date} params.expiration - Expiration date
 * @param {string} params.principalUuid - Principal UUID
 * @returns {Object} LDAP object for UFDS
 */
function buildLdapObjectForSessionToken(params) {
    var obj = {
        objectclass: ['accesskey'],
        ...
        principaluuid: params.principalUuid,
        credentialtype: 'temporary',
        status: 'Active',
        created: Date.now().toString(),
        updated: Date.now().toString()
    };
    var sessionTokenScopeAttr = ldapScopeAttr(params.bucketScope);
    if (sessionTokenScopeAttr) {
        obj.accesskeyscope = sessionTokenScopeAttr;
    }
    return (obj);
}

/**
 * Builds access key data object for Redis storage.
 *
 * @param {Object} params - Parameters for access key data
 * @param {string} params.accessKeyId - Temporary access key ID
 * @param {string} params.secretAccessKey - Secret access key
 * @param {string} params.sessionToken - JWT session token
 * @param {string} params.userUuid - User UUID
 * @param {Date} params.expiration - Expiration date
 * @param {string} params.principalUuid - Principal UUID
 * @returns {Object} Access key data for Redis
 */
function buildAccessKeyDataForRedis(params) {
    var data = {
        type: 'accesskey',
        accessKeyId: params.accessKeyId,
    ...
    if (params.bucketScope) {
        data.bucketScope = params.bucketScope;
    }
    return (data);
}

/**
 * Builds GetSessionToken API response.
 *
 * @param {Object} credentials - Temporary credentials
 * @param {string} credentials.accessKeyId - Access key ID
 * @param {string} credentials.secretAccessKey - Secret access key
 * @param {string} credentials.sessionToken - Session token
 * @param {Date} credentials.expiration - Expiration date
 * @returns {Object} GetSessionToken response
 */
function buildGetSessionTokenResponse(credentials) {
    return {
...
function getSessionToken(req, res, next) {
    ...
    var caller = req.body.caller;
    var callerIdentity = extractCallerIdentity(caller);
    var callerUuid = callerIdentity.uuid;
    var callerLogin = callerIdentity.login;
    /*
     * Contract: manta-buckets-api sends a JSON scope string
     * or null/undefined. Fail fast on any other type so the
     * buckets-api regression is visible instead of silently
     * corrupting credentials. Hand-rolled because the shipped
     * assert-plus (0.1.5) optionalString rejects null.
     */
    if (caller.bucketScope !== undefined && caller.bucketScope !== null) {
        assert.string(caller.bucketScope, 'caller.bucketScope');
    }
    ...
    var durationValidation = validateDurationSeconds(req.params, req.body);
    if (!durationValidation.valid) {
        next(durationValidation.error);
        return;
    }
    var durationSeconds = durationValidation.value;
    ...
    var sessionTokenData = createSessionTokenData(callerUuid, expiration);
    ...
    var ldapObject = buildLdapObjectForSessionToken({
        accessKeyId: tempAccessKeyId,
    ...
    req.ufdsPool.acquire(function (poolErr, ufdsClient) {
        ...
        var accessKeyData = buildAccessKeyDataForRedis({
            accessKeyId: tempAccessKeyId,
            secretAccessKey: tempSecretKey,
            sessionToken: secureSessionToken,
            userUuid: callerUuid,
        ...
        // Return STS response
        var response = buildGetSessionTokenResponse({
            accessKeyId: tempAccessKeyId,
            secretAccessKey: tempSecretKey,
            sessionToken: secureSessionToken,
            expiration: expiration
...
/**
 * Constructs an ARN for a federated user session.
 *
 * @param {string} arnPartition - ARN partition (e.g., 'aws', 'aws-us-gov')
 * @param {string} uuid - User or account UUID
 * @param {string} login - User or account login name
 * @returns {string} Federated user ARN
 */
function constructFederatedUserArn(arnPartition, accountUuid, login) {
    return ('arn:' + arnPartition + ':sts::' + accountUuid +
           ':federated-user/' + login);
}


/**
 * Constructs an ARN for an IAM user.
 *
 * @param {string} arnPartition - ARN partition (e.g., 'aws', 'aws-us-gov')
 * @param {string} accountUuid - Account UUID
 * @param {string} login - User or account login name
 * @returns {string} IAM user ARN
 */
function constructIAMUserArn(arnPartition, accountUuid, login) {
    return ('arn:' + arnPartition + ':iam::' + accountUuid + ':user/' + login);
}
...
function getCallerIdentity(req, res, next) {
    ...
    if (req.auth && isTemporaryCredential && assumedRole) {
        ...
        arn = constructFederatedUserArn(arnPartition, userId,
        ...
        userId = (user && user.uuid) || (account && account.uuid);
        arn = constructIAMUserArn(arnPartition,
            (account && account.uuid) || (user && user.uuid),
            (user && user.login) || (account && account.login));
...
module.exports = {
    ...
    // Export helper functions for testing (CHG-044)
    helpers: {
        extractCallerIdentity: extractCallerIdentity,
        validateDurationSeconds: validateDurationSeconds,
        createSessionTokenData: createSessionTokenData,
        buildLdapObjectForSessionToken: buildLdapObjectForSessionToken,
        buildAccessKeyDataForRedis: buildAccessKeyDataForRedis,
        buildGetSessionTokenResponse: buildGetSessionTokenResponse,
        parseRoleArnForTrustPolicy: parseRoleArnForTrustPolicy,
        findTrustPolicyInMemberpolicy: findTrustPolicyInMemberpolicy
    }
};
