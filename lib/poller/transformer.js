...
function addToSet(opts, cb) {
    ...
    redis.get(key, function gotValue(err, res) {
        ...
        if (payload[set].indexOf(element) === -1) {
            payload[set].push(element);
        }
...
function delFromSet(opts, cb) {
    ...
    redis.get(key, function gotValue(err, res) {
        ...
        payload[set] = payload[set] || [];
        index = payload[set].indexOf(element);
        if (index > -1) {
            ...
            if (change.modification.type === 'role') { // rename
                ...
                if (change.operation === 'add') {
                    setUnion({
                        key: '/uuid/' + uuid,
                        set: 'policies',
                        elements: change.modification.vals,
                        batch: batch,
                        log: self.log,
                        redis: self.redis
                    }, pipelinecb);
                    ...
                    setValue({
                        key: '/uuid/' + uuid,
                        property: 'policies',
                        value: change.modification.vals,
                        batch: batch,
                        log: self.log,
                        redis: self.redis
                    }, pipelinecb);
...
Transformer.prototype.modUser = function (opts, cb) {
    ...
    var login = opts.modEntry.login[0];
    var key = '/uuid/' + uuid;
