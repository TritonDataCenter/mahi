...
var CHANGELOG_COUNT = 'cn=changelogcount';
var CHANGELOG_COUNT_FILTER = '(&(objetclass=*))';
...
function AuthCache(options) {
    ...
    redis.createClient(options.redisCfg, function(err, redisClient) {
        ...
        self.redisClient.get('changenumber', function(err, res) {
            redisErrHandler(self, err);
            // MANTA-1565 see if virgin was still set from the previous run --
            // in case the initial run was interrupted
            self.redisClient.get('virgin', function(err, res) {
                redisErrHandler(self, err);
                if (res) {
                    self.isVirgin = true;
                }
            ...
            if (!res) {
                self.changenumber = 0;
                // MANTA-1429 this means that this cache is a virgin, so we
                // need to write // this state in redis
                self.isVirgin = true;
                log.info('virgin cache, setting virgin flag in redis');
                self.redisClient.set('virgin', 'true', function(err) {
                    redisErrHandler(self, err);
                });
            } else {
...
function poll(self) {
    ...
    function pollOnceCb(err, gotEntries) {
        ...
        if (gotEntries) {
            ...
            // fix for MANTA-1583
            getLatestChangenumber(self, function(err, cn) {
                if (err) {
                    log.error({err: err}, 'could not get latest ldap cn, ' +
                              'stopping polling until next timeout');
                    self.isPolling = false;
                } else if (cn > self.changenumber) {
                    log.info({
                        ldapcn: cn,
                        localcn: self.changenumber
                    }, 'continue polling since there are more records in ldap');
                    pollOnce(self, pollOnceCb);
                } else {
                    // MANTA-1429, remove virgin flag once we stop polling
                    // until timeout, indicating there's no more work to be done
                    if (self.isVirgin) {
                        log.info({
                            cacheCn: self.changenumber,
                            ldapCn: cn
                        }, 'cache caught up, removing virgin flag');

                        self.redisClient.del('virgin', function(err) {
                            redisErrHandler(self, err);
                            self.isVirgin = false
                        });
                    }
...
function pollOnce(self, cb) {
    ...
    /* JSSTYLED */
    var filter = sprintf('(&(changenumber>=%d)(|(targetdn=*ou=users*)(targetdn=*ou=groups*))(!(targetdn=vm*))(!(targetdn=amon*)))', start);
...
function getLatestChangenumber(self, cb) {
    var log = self.log;
    log.info('getLatestChangenumber: entering');

    var latestCn;
    var opts = {
        filter: CHANGELOG_COUNT_FILTER
    };
    self.ldapClient.search(CHANGELOG_COUNT, opts, function(err, res) {
        res.on('searchEntry', function(entry) {
            latestCn = parseInt(entry.object.count, 10);
            log.info({lattestCn: latestCn}, 'getLatestChangenumber: got cn');
        });
        res.once('error', function(err) {
            log.error({err: err},
                'encountered ldap error, aborting current poll');
    ...
    client.get(uuidKey, function(err, oldLogin) {
        ...
        if (oldLogin === newLogin) {
            log.info({
                oldLogin: oldLogin,
                newLogin: newLogin
            }, 'updateLogin: new and old logins are the same, skipping record');
            cb();
            return;
        }
