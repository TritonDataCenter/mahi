...
function poll(self) {
  ...
  var max = start + 100;
  /* JSSTYLED */
  var filter = sprintf('(&(changenumber>=%s)(|(targetdn=*ou=users*)(targetdn=*ou=groups*))(!(targetdn=vm*))(!(targetdn=amon*)))', start);
  var opts = {
    scope: 'sub',
    filter: filter
  };
  ...
  var timeoutId = setTimeout(onTimeout, self.timeout);
  ...
  self.ldapClient.search(CHANGELOG, opts, function(err, res) {
    ...
    res.on('searchEntry', function(entry) {
      ...
      if (targetdn.childOf(USER_DN) || targetdn.childOf(GROUP_DN)) {
        log.info('dn matches', entry.object);
        ...
        if (changes && changes.objectclass) {
          ...
          // the group.  Only check objectclass once we're sure we have a mod
          // entry
    ...
    res.on('error', function(err) {
      clearTimeout(timeoutId);
      log.error({err: err}, 'encountered ldap error, aborting current poll');
      self.currPolling = false;
  ...
  function onTimeout() {
    log.error('ldap request timedout');
    if (timeoutId._ldapRes) {
      log.info('removing all listeners from current search res');
      timeoutId._ldapRes.removeAllListeners();
    }

    log.info({
            currPolling: self.curPolling
    }, 'setting curPolling back to false');
    self.currPolling = false;
  }
}
...
function mod(self, changes, entry, cb) {
  ...
  changes.forEach(function(change, index) {
    log.info('modify changelog: ', change);
    if (change.modification.type === 'uniquemember') {
      change.modification.vals.forEach(function(userdn) {
...
function add(self, changes, entry, cb) {
  ...
  function addUser(self, changes, entry, cb) {
    var log = self.log;
    log.info('adding user', changes, entry.object.changenumber);
    var client = self.redisClient;
    ...
    // also add login and uuid to their respective sets
  ...
  function addKey(self, changes, entry, cb) {
    var log = self.log;
    log.info('adding key', changes, entry.object.changenumber);
  ...
  function addGroup(self, changes, entry, cb) {
    var log = self.log;
    log.info('adding group', changes, entry.object.changenumber);
...
function del(self, changes, entry, cb) {
  ...
  function delKey(self, changes, entry, cb) {
    var log = self.log;
    log.info('deleting key', changes, entry.object.changenumber);
    // like fingerprint=foo, uuid=bar, ou=users, o=smartdc
    var myDn = entry.object.targetdn;
