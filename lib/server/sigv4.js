        ...
        for (var partIdx = 0; partIdx < parts.length; partIdx++) {
                ...
                if (keyValue.length === 2) {
                        ...
                        if (key === 'Credential') {
                                ...
                                /*
                                 * Validate credential format per AWS SigV4 spec:
                                 * Credential=AccessKeyId/DateStamp/Region/Service/RequestType
                                 * All 5 parts are required and must be non-empty.
                                 */
                                ...
                                /*
                                 * Validate accessKeyId format per sdc-ufds schema:
                                 * - Must contain only word characters (alphanumeric + underscore)
...
function verifySigV4(opts, cb) {
        ...
        log.debug({
                ...
                accessKeyIdHex: (authInfo.accessKeyId &&
                     ...
                     (function () {
                        if (authInfo.accessKeyId.length > 1024) {
                             throw new Error('Access key ID too long');
                         }
                         return new Buffer(
    ...
    var isTemporaryCredential = sessionToken &&
        ...
        if (isTemporaryCredential) {
            ...
            var sessionToken_module = require('./session-token');
            ...
            sessionToken_module.verifySessionToken(sessionToken,
                                                   secretConfig, {},
                                                   function (jwtErr,
                                                             tokenData) {
                if (jwtErr) {
