    ...
    transform.add(args, function (err, res) {
        ...
        res.exec(function (err) {
                ...
                REDIS.get(key, function (err, userRes) {
                    ...
                    t.equal(
                            userPayload.accesskeys['AKIA123456789EXAMPLE'],
                            'wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY',
                            'access key secret should be stored correctly');
                // Check reverse lookup
                REDIS.get(accessKeyLookupKey,
    ...
    var entry = {
        ...
        res.exec(function (err) {
                ...
                REDIS.get(key, function (err, userRes) {
                        ...
                        t.equal(
                        userPayload.accesskeys['AKIASUBUSER000000001'],
                    'subuserSecretKeyForTesting123456789abcdef',
    ...
    transform.add(args, function (err, res) {
        ...
        res.exec(function (err) {
            ...
            REDIS.get(key, function (err, userRes) {
                    ...
                    t.equal(
                            userPayload.accesskeys['AKIA987654321EXAMPLE'],
                            'abcdefghijklmnopqrstuvwxyz1234567890ABCD',
...
test('add - access key with special characters', function (t) {
    ...
    REDIS.get(key, function (err, userRes) {
            ...
            var userPayload = JSON.parse(userRes);
            t.equal(
    userPayload.accesskeys['AKIASPECIALCHARS0001'],
    specialSecret,
       'access key with special characters should be stored correctly');
    t.done();
    });
    });
});
        ...
        res.exec(function (err) {
            ...
            REDIS.get(key, function (err, userRes) {
                    ...
                    REDIS.get(accessKeyLookupKey,
                            function (err, lookupRes) {
                            t.ok(!err,
                                    'reverse lookup get should not error');
            ...
            res.exec(function (err) {
                    ...
                    REDIS.get(key, function (err, userRes) {
                            ...
                            t.ok(userPayload.accesskeys,
                                    'user should still have accesskeys object');
                            t.equal(
                                    Object.keys(userPayload.accesskeys).length,
                                    0,
                                    'user should have no access keys');
                            t.done();
                    });
                    });
            });
...
test('modify - noop', function (t) {
        // According to accesskey.js, modify is a NOP
        transform.modify({
                redis: REDIS
        }, function (err, res) {
                t.ok(!err, 'modify should not error');
                t.ok(res, 'modify should return a redis multi');
                t.equal(res.queue.length, 1, 'modify should be a no-op');
                t.done();
        });
});
