...
function assumeRole(req, res, next) {
    ...
    log.info('sts.assumeRole: entered');

    // Caller should be set by server wrapper from headers
    var caller = req.caller;
    if (!caller || !caller.uuid) {
        res.send(400, {error: 'Caller identity required'});
        return (next());
    }

    var callerUuid = caller.uuid;
    var callerLogin = caller.account ? caller.account.login : null;
        ...
        accesskey.generate(accesskey.DEFAULT_PREFIX,
            ...
            try {
                ...
                if (!req.sessionConfig ||
                    req.sessionConfig.enabled !== 'true') {
...
function getSessionToken(req, res, next) {
    ...
    try {
        ...
        if (!req.sessionConfig || req.sessionConfig.enabled !== 'true') {
            throw new errors.ServiceUnavailableError(
                'STS operations are not enabled');
        }
