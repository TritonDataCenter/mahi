...
function AuthCache(options) {
  ...
  */
  ...
  self.ldapClient.bind(options.ldapCfg.bindDN,
                       options.ldapCfg.bindCredentials, function(err)
  {
    if (err) {
      log.fatal('unable to bind', err);
      process.exit(err);
    }
    log.info('ldap client bound!', err);
  });

  self.ldapClient.on('error', function(err) {
    log.fatal('ldap client error', err);
    process.exit(err);
  });
