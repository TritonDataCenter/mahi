...
function add(opts, cb) {
    ...
    redis.get(key, function (err, res) {
        ...
        /*
         * `version` in the stored entry records the UFDS
         * changenumber of the write that produced it. It is
         * informational only since CHG-138 removed the
         * external cachePush writer that used to compete with
         * the replicator on the same key; the replicator is
         * now the sole producer, its changenumbers are
         * monotonic by construction, and there is no
         * conflict to resolve. Compute defensively so the
         * field stays consistent even when changelog entries
         * lack a parseable changenumber.
         */
...
function modify(opts, cb) {
    ...
    redis.get(key, function _redisGet(err, res) {
        ...
        /* See add() above for the version-field rationale. */
        var changenumber = parseInt(entry.changenumber, 10);
        if (!Number.isFinite(changenumber) || changenumber <= 0) {
            changenumber = 0;
        }
