        ...
        res.on('end', function(res) {
            ...
            if (entries.length === 0) {
                ...
                if (self.changenumber < latestChangenumber) {
                    ...
                    /*
                     * if there are no entries, update the cn in redis to the
                     * latest one seen during this round of searches.
                     * Otherwise, the cn doesn't get updated, which causes mahi
                     * to continue searching from the old cn.
                     */
            ...
            entries.forEach(function(entry, index) {
                ...
                if (index === entries.length - 1) {
                    log.info('finished pushing changelogs up to %s',
                        self.changenumber);
                    self.currPolling = false;
                }
            });
...
function add(self, changes, entry, cb) {
    ...
    function addUser(self, changes, entry, cb) {
        ...
        var payload = {
            uuid: changes.uuid[0],
            // MANTA-1289
            approved_for_provisioning: changes.approved_for_provisioning ?
