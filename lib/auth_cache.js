...
function poll() {
    var self = this;
    pollOnce.call(self, function (err, numUpdated) {
        if (err) {
            self.log.error(err, 'Error when polling');
        }
        //Don't sleep if we actually had updates.
        var pollInterval = self.pollInterval;
        if (numUpdated !== undefined && numUpdated > 0) {
            pollInterval = 1;
        }
        setTimeout(poll.bind(self), pollInterval);
    });
}
...
function pollOnce(cb) {
    ...
    // Why do we parseint, then use a %s in the query?
    var start = parseInt(self.changenumber, 10);

    //The default size limit returned from ldap is 1000.  Variablifying it
    // so that we don't mess up the limit in the filter.
    var limit = 1000;

    /* JSSTYLED */
    var filter = sprintf('(&(changenumber>=%s)(changenumber<=%s)' +
                           '(|(targetdn=*ou=users*)' +
                             '(targetdn=*ou=groups*))' +
                           '(!(targetdn=vm*))' +
                           '(!(targetdn=amon*))' +
...
function updateChangeNumber(self, changenumber, log, cb) {
    if (self.changenumber < changenumber) {
        self.changenumber = changenumber + 1;
...
function del(self, changes, entry, cb) {
    ...
    case 'groupofuniquenames':
        ...
        break;
    default:
        cb(new Error('default case invokded.'));
    }
