 ...
 * Redis (via the replicator), the cache entry will still
 * cause 401s for up to NEGATIVE_CACHE_TTL_MS (30 seconds).
 *
 ...
 *
 * Mitigation: the UFDS read-through (see
 * handlePermanentCredentialUfds below) resolves the new key
 * directly from UFDS on the first Redis miss, before this
 * negative cache has anything to add. The read-through is
 * what makes newly created keys usable immediately. The
 * negative cache exists to bound the cost of repeated
 * lookups for keys that are genuinely invalid.
 */
...
function handlePermanentCredentialUfds(
    ...
    /*
     * Validate access key ID per the sdc-ufds schema before using
     * it in an LDAP filter. Reject anything outside the allowed
     * accesskeyid character set and length bounds to prevent
     * filter injection and keep this path consistent with the
     * rest of SigV4 credential parsing.
     */
    if (!ACCESSKEYID_RE.test(authInfo.accessKeyId) ||
        authInfo.accessKeyId.length < MIN_ACCESSKEYID_LENGTH ||
    ...
    ufds.search(searchBase, {
        ...
        /*
         * Owner UUID is encoded in the entry's DN
         * (accesskeyid=K, uuid=OWNER, ou=users, o=smartdc).
         * `_owner` is a convention populated by some sdc-clients
         * helpers but not by raw ufds.search(), so prefer the DN
         * and fall back to _owner for any callers that already
         * set it.
         */
        var dn = cred.dn || cred.objectName || '';
        var dnMatch = (typeof (dn) === 'string') ?
            dn.match(/uuid=([^,]+)/i) : null;
        var ownerUuid = (dnMatch && dnMatch[1]) ||
            credData._owner;
 ...
 * verification.
 *
 * Shared by handleTemporaryCredentialRedis and handleTemporaryCredential (UFDS)
 * to prevent divergence in result construction in the permanent credential
 * path.
 * @param {Object} opts
 ...
 * verification.
 *
 * Shared by handlePermanentCredentialRedis and handlePermanentCredentialUfds to
 * prevent divergence in result construction.
 *
