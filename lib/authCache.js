...
function AuthCache(options) {
  ...
  assertObject('options.redisCfg', options.redisCfg);
  ...
  * ldap client
  */
  this.ldapClient = ldap.createClient(options.ldapCfg);
  ...
  */
  ...
  setInterval(poll, self.pollInterval, self);
  poll(self);
...
function poll(self) {
  var log = self.log;
  if (self.currPolling) {
    log.info('already polling, aborting.');
    return;
  }

  self.currPolling = true;

  var filter = sprintf('(&(targetdn=*)(changenumber>=%s))', self.changenumber);
  var opts = {
    scope: 'sub',
    filter: filter
  };
  ...
  self.ldapClient.search(CHANGELOG, opts, function(err, res, count) {
    ...
    res.on('searchEntry', function(entry) {
      log.info('search entry', entry.object.targetdn,
      ...
      if (targetdn.childOf(USER_DN) || targetdn.childOf(GROUP_DN)) {
        ...
        if (changes.objectclass) {
          var objectclass =
          changes.objectclass[0];
          if (objectclass === 'sdcperson' || objectclass === 'sdckey' ||
            ...
            entries.push(entry);
                   ...
                   entry.object.changetype === 'groupofuniquenames') {
          // modify doesn't have objectclass
          entry.parsedChanges = changes;
          entries.push(entry);
        }
      }
    ...
    res.on('end', function(res) {
      ...
      entries.forEach(function(entry, index) {
        ...
        if (index === entries.length - 1) {
          ...
          // +1 because changenumber filters require >= not >
...
function parseEntry(entry, cb) {
  ...
  //log.debug('pushing entry', entry.object.changenumber, changetype, targetdn);
  var changenumber = entry.object.changenumber;
  log.info('parsing entry', changenumber);
  switch(changetype) {
    // adds contain the object class as part of the change field.
  case 'add':
    add(self, entry.parsedChanges, entry, cb);
    break;
    // we must check the entry field
  case 'modify':
    // only for group updates
    // modifies don't contain the object class in the change field
    mod(self, entry.parsedChanges, entry, cb);
    break;
    // add all deletes since deletes don't have objectclass at all
  case 'delete':
    //log.info('targetdn', targetdn);
...
// only process objectclass=groupofuniquenames which is for
    ...
    if (change.modification.type === 'uniquemember') {
      switch(change.operation) {
        case 'add':
          ...
          addGroupMember(self, user, groupdn, function() {
          });
          break;
        case 'delete':
          removeGroupMember(self, change, entry, function() {
          });
          break;
        default:
          throw new Error('default case invoked');
...
function removeGroupMember(self, change, entry, cb) {
  var log = self.log
  var member = change.modification.vals[0];

  var client = self.redisClient;
  var targetdn = entry.object.targetdn;
  var group = targetdn.substring(targetdn.indexOf('=') + 1,
                                 targetdn.indexOf(','));
  var userUuid = member.substring(member.indexOf('=') + 1, member.indexOf(','));
  // get the login from uuid
  var loginKey = sprintf('/uuid/%s', userUuid);
  log.info('loginkey', loginKey);
  var client = self.redisClient;
...
function add(self, changes, entry, cb) {
  ...
  function addUser(self, changes, entry, cb) {
    ...
    client.multi().set(key, JSON.stringify(payload))
    .client.set(uuidKey, login, function() {
      return cb();
    });
  }
...
function delUser(self, changes, entry, cb) {
  var log = self.log;
  log.debug('deleting user', changes, entry.object.changenumber);
  var client = self.redisClient;
  var login = changes.login[0];

  var key = sprintf('/login/%s', login);
  var uuidKey = sprintf('/uuid/%s', changes.uuid);
  client.multi().del(key)
  .del(uuidKey, function() {
    return cb();
  });
  ...
  function delGroup(self, changes, entry, cb) {
    ...
    log.info('group entry', entry.object);
    // like uuid=930896af-bf8c-48d4-885c-6573a94b1853, ou=users, o=smartdc
    var myDn = changes.uniquemember[0];
    var userUuid = myDn.substring(myDn.indexOf('=') + 1, myDn.indexOf(','));

    // get group name from targedn cn=operators, ou=groups, o=smartdc
    var targetdn = entry.object.targetdn;
    var group = targetdn.substring(targetdn.indexOf('=') + 1,
    targetdn.indexOf(','));
    log.info('group name is', group);
    ...
    client.get(loginKey, function(err, login) {
      ...
      client.get(key, function (err, payload) {
        ...
        delete payload.groups[group];
        payload = JSON.stringify(payload);
        log.info('deleting group entry', key, payload);
        client.set(key, payload, function(err) {
          redisErrHandler(self, err);
          return cb();
        });
