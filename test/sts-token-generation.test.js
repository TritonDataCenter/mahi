...
/**
 * test/sts-token-generation.test.js: Unit tests for STS token
 * generation
 *
...
var generateAssumeRoleAccessKeyId =
    sts.internal.generateAssumeRoleAccessKeyId;
...
exports.testSessionTokenJWTStructure = function (t) {
    ...
    t.equal(decoded.uuid, sessionData.uuid,
        'should contain user UUID');
    t.equal(decoded.roleArn, sessionData.roleArn,
        'should contain role ARN');
...
exports.testSessionTokenJWTSignature = function (t) {
    ...
    jwt.verify(token, secretKey.key, function (err, verified) {
        t.ifError(err,
            'JWT signature should verify with correct secret');
        ...
        jwt.verify(token, wrongSecret, function (verifyErr, result) {
            ...
            t.ok(!result,
                'should not return payload with wrong secret');
            t.done();
        });
    });
};
...
exports.testJWTPayloadEncoding = function (t) {
    ...
    t.equal(payload.uuid, sessionData.uuid,
        'payload should contain UUID');
    t.equal(payload.roleArn, sessionData.roleArn,
        'payload should contain role ARN');
    t.equal(payload.sessionName, sessionData.sessionName,
        'payload should contain session name');
    t.equal(payload.exp, sessionData.expires,
        'payload should contain expiration');
    t.ok(payload.iat, 'payload should contain issued-at timestamp');
    t.ok(payload.nbf,
        'payload should contain not-before timestamp');
    t.equal(payload.iss, 'test-issuer',
        'payload should contain issuer');
    t.equal(payload.aud, 'test-audience',
        'payload should contain audience');
...
exports.testTokenExpirationCalculation = function (t) {
    ...
    var decoded = jwt.decode(token);
    t.equal(decoded.exp, expectedExpiration,
        'expiration should match calculated value');

    // Verify iat (issued at) is approximately now
    t.ok(Math.abs(decoded.iat - now) <= 1,
        'issued-at should be approximately now');

    // Verify nbf (not before) is approximately now
    t.ok(Math.abs(decoded.nbf - now) <= 1,
        'not-before should be approximately now');
...
exports.testDurationValidationMinimum = function (t) {
    ...
    // Test short duration (token generation doesn't enforce
    // AWS 900s minimum)
    // The 900 second minimum is enforced at the STS API level,
    // not token level
    var sessionData = {
    ...
    // Should generate successfully - token module doesn't
    // enforce minimum
    var token = sessionTokenModule.generateSessionToken(sessionData,
        secretKey);
    t.ok(token, 'should generate token with short duration');

    var decoded = jwt.decode(token);
    t.equal(decoded.exp, sessionData.expires,
        'should have correct expiration');
...
exports.testDurationValidationPastExpiration = function (t) {
    ...
    t.throws(function () {
        ...
        'should reject expiration in the past');
...
exports.testSessionTokenAccessKeyIdGeneration = function (t) {
    ...
    t.ok(/^[A-F0-9]{16}$/.test(hexPart),
        'should contain 16 uppercase hex characters');
...
exports.testSecretKeyGeneration = function (t) {
    // Use accesskey.generate to create secret key
    // (as used in STS implementation)
    accesskey.generate(accesskey.DEFAULT_PREFIX,
        ...
        function (err, secretKey) {
            ...
            t.equal(typeof (secretKey), 'string',
                'secret key should be a string');

            // Secret key should be base64-encoded and have
            // appropriate length
            t.ok(secretKey.length > 40,
                'secret key should have sufficient length');
...
exports.testSecretKeyRandomness = function (t) {
    ...
    for (var i = 0; i < iterations; i++) {
        accesskey.generate(accesskey.DEFAULT_PREFIX,
            ...
            function (err, secretKey) {
                t.ifError(err,
                    'should not error generating secret key');
                keys.push(secretKey);
                remaining--;
...
exports.testSessionTokenStringFormat = function (t) {
    ...
    t.ok(jwtPattern.test(token),
        'token should match JWT format pattern');

    // Verify base64url encoding (no +, /, or = characters)
    t.equal(token.indexOf('+'), -1,
        'should not contain + character (base64url)');
    t.equal(token.indexOf('/'), -1,
        'should not contain / character (base64url)');
    t.equal(token.indexOf('='), -1,
        'should not contain = character (base64url)');
...
exports.testErrorInvalidDataTypes = function (t) {
    ...
    t.throws(function () {
        sessionTokenModule.generateSessionToken(invalidUuidData,
            secretKey);
    ...
    t.throws(function () {
        sessionTokenModule.generateSessionToken(invalidExpiresData,
            secretKey);
...
exports.testErrorZeroExpiration = function (t) {
    ...
    t.throws(function () {
        ...
        'should throw error for zero expiration');
...
exports.testErrorNegativeExpiration = function (t) {
    ...
    t.throws(function () {
        ...
        'should throw error for negative expiration');
