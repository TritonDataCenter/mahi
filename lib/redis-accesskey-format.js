...
function buildPermanentKeyEntry(secret, scope, version) {
    /*
     * Validate scope JSON if present.  Reject malformed
     * values at the write boundary rather than storing
     * garbage in Redis where every downstream consumer
     * would need to handle the parse error.
     */
    if (scope != null && scope !== '') {
        try {
            JSON.parse(scope);
