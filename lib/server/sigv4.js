 ...
 *
 * PLATFORM LIMITATION: This module is NOT Y2038-compliant when running on
 * 32-bit Node.js binaries. After 2038-01-19 03:14:08 UTC, timestamp
 * validation will fail due to 32-bit time_t overflow in the underlying
 * platform. The module will log errors when Y2038 requests are detected
 * and skip timestamp freshness validation to allow authentication to
 * continue (with degraded replay attack protection).
 *
 * Solution: Deploy on 64-bit Node.js binary before 2038-01-19.
 */
 ...
 * Used to detect when running on 32-bit platform after Y2038 overflow point.
 * Milliseconds since Unix epoch: 2^31 * 1000
 */
var Y2038_THRESHOLD_MS = 2147483648000;
...
function handleTemporaryCredential(authInfo, sessionToken, req, log, ufds, cb) {
    ...
    ufds.search(searchBase, {
        ...
        redis.get(userKey, function (err, userRes) {
            ...
            // Y2038 detection: On 32-bit platforms, Date.now() overflows
            // after 2038-01-19. Skip validation and log error.
            if (requestTime > Y2038_THRESHOLD_MS) {
                log.error({
                    ...
                    timeDiff: timeDiff
                }, 'Y2038: Cannot validate timestamp freshness on 32-bit ' +
                   'platform. Request allowed without replay protection.');
    ...
    var isTemporaryCredential = sessionToken &&
        ...
        redis.get(accessKeyLookupKey, function (err, userUuid) {
                ...
                redis.get(userKey, function (err, userRes) {
                        ...
                        // Y2038 detection: On 32-bit platforms, Date.now()
                        // overflows after 2038-01-19. Skip validation and
                        // log error.
                        if (requestTime > Y2038_THRESHOLD_MS) {
                                log.error({
                                        requestTimestamp: timestamp,
                                        ...
                                        systemTimeMs: currentTime,
                                        timeDiff: timeDiff
                                }, 'Y2038: Cannot validate timestamp ' +
                                   'freshness on 32-bit platform. ' +
                                   'Request allowed without replay ' +
                                   'protection.');
                        } else if (timeDiff > 15 * 60 * 1000) { // 15 minutes
                                cb(new errors.InvalidSignatureError(
                                        'Request timestamp too old'));
