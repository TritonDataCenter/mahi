    ...
    Redis.createClient(redisConfig, function (err, redis) {
        ...
        http.createServer(function (request, response) {
            redis.ping(function (err) {
                ...
                redis.get('virginv2', function (err, res) {
                    if (err) {
        ...
        redis.get('changenumberv2', function (err, changenumber) {
            if (err) {
                log.fatal({err: err}, 'error getting changenumber from redis');
                process.exit(1);
            }

            redis.set('virginv2', 'true', function (err) {
                if (err) {
                ...
                reader.once('fresh', function onFresh() {
                    redis.del('virginv2', function delVirgin(err) {
                        if (err) {
                            log.error({err: err}, 'error removing virgin flag');
                            return;
                        }
