    ...
    req.redis.get('/account/' + account, function (err, uuid) {
        ...
        req.redis.get('/uuid/' + uuid, function (err, blob) {
            ...
            req.auth.account.isOperator = req.auth.account.groups &&
                req.auth.account.groups.indexOf('operators') >= 0;
            next();
            return;
        });
    });
...
function getUser(req, res, next) {
    ...
    req.redis.get(key, function (err, uuid) {
        ...
        if (!uuid) {
            /* send what information we have in addition to returning the 404 */
            next(new errors.UserDoesNotExistError(user, account.login,
                req.auth));
            return;
        }
        req.redis.get('/uuid/' + uuid, function (err, blob) {
...
function getName(req, res, next) {
    ...
    vasync.forEachParallel({
        func: function forEachUuid(uuid, cb) {
            req.redis.get('/uuid/' + uuid, function (err, info) {
                if (err) {
                    cb(err);
                    return;
                }
