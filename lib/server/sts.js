    ...
    function generateTemporaryCredentials(roleData) {
        ...
        accesskey.generate(accesskey.DEFAULT_PREFIX,
            ...
            try {
                ...
                var buildSecretConfig = serverModule.buildSecretConfig;
                ...
                log.debug({
                    ...
                    hasSecretKey: !!secretConfig.primarySecret,
                    secretKeyHasKey: secretConfig.primarySecret &&
                        !!secretConfig.primarySecret.key,
                    secretKeyHasKeyId: secretConfig.primarySecret &&
                        !!secretConfig.primarySecret.keyId,
                    tokenOptions: tokenOptions
                }, 'Calling generateSessionToken');
                ...
                log.error({
                    ...
                    errorName: tokenErr.name,
                    errorStack: tokenErr.stack,
                    errorString: String(tokenErr),
                    roleArn: roleArn,
                    sessionName: roleSessionName
...
function extractCallerIdentity(caller) {
    var uuid = caller.user && caller.user.uuid ? caller.user.uuid :
        caller.account.uuid;
    var login = caller.user && caller.user.login ? caller.user.login :
        caller.account.login;
    return {uuid: uuid, login: login};
}
 ...
 */
function constructFederatedUserArn(arnPartition, uuid, login) {
    return 'arn:' + arnPartition + ':sts::' + uuid +
